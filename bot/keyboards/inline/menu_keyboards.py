...
def make_callback_data(level=2, category="0", subcategory="0", item_id="0"):
    return menu_cd.new(
...
async def subproduct_keyboard(category):
    CURRENT_LEVEL = 1
    markup = InlineKeyboardMarkup(row_width=1)

    # Kategoriya ostidagi kategoriyalarni bazadan olamiz
    subproduct = await db.get_subproduct(category)
    for subcategory in subproduct:
...
async def items_keyboard():
    ...
    for item in items:
        ...
        # Tugma bosganda qaytuvchi callbackni yasaymiz: Keyingi bosqich +1 va kategoriyalar
        callback_data = make_callback_data(3,
            item_id=item["id"],
        )
        markup.insert(
            InlineKeyboardButton(text=button_text, callback_data=callback_data)
